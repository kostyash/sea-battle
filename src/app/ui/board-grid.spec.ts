...
describe('BoardGrid', () => {
  ...
  /**
   * The Admiral's reckoning, washed onto the paper. It is shown only after a
   * battle, over a chart the player already knows — during play it would be
   * telling them where their own ships are and what the opponent is about to do.
   */
  describe("the opponent's reckoning", () => {
    const washes = (): number[] =>
      Array.from(fixture.nativeElement.querySelectorAll('.reckoning span'), (span: Element) =>
        Number((span as HTMLElement).style.opacity),
      );

    it('is not on the chart at all until there is one to show', () => {
      render(emptyBoard('player'));
      expect(fixture.nativeElement.querySelector('.reckoning')).toBeNull();
    });

    it('washes every square of the board once a reckoning is handed to it', () => {
      const score = Array.from({ length: CELLS }, (_, i) => i);
      render(emptyBoard('player'), { heat: score });

      expect(washes()).toHaveLength(CELLS);
    });

    /**
     * Stretched between the coldest and warmest square rather than scaled from
     * zero: early on every square is worth about the same, and a scale from zero
     * floods the whole chart one flat colour with nothing to read off it.
     */
    it('spends the whole ramp on the range that is actually there', () => {
      const score = new Array<number>(CELLS).fill(1000);
      score[idx(0, 0)] = 1000;
      score[idx(5, 5)] = 1006;
      render(emptyBoard('player'), { heat: score });

      const shown = washes();
      expect(shown[idx(0, 0)]).toBe(0);
      expect(shown[idx(5, 5)]).toBeGreaterThan(0);
    });

    it('shades in steps, so a maximum can be picked out rather than blurring', () => {
      const score = Array.from({ length: CELLS }, (_, i) => i);
      render(emptyBoard('player'), { heat: score });

      const steps = new Set(washes().map((o) => o.toFixed(4)));
      expect(steps.size).toBeGreaterThan(1);
      expect(steps.size).toBeLessThanOrEqual(7);
    });

    it('leaves a square already fired at clear — there was nothing to reckon about it', () => {
      const board = fire(withShip(emptyBoard('player'), 0, 0, 1, 'h'), idx(9, 9)).board;
      const score = new Array<number>(CELLS).fill(5);
      render(board, { heat: score });

      expect(board.shots[idx(9, 9)]).toBe('miss');
      expect(washes()[idx(9, 9)]).toBe(0);
    });

    it('reads the marks as they stood at that salvo, not as the battle ended', () => {
      const ended = fire(emptyBoard('player'), idx(4, 4)).board;
      const asOf = new Array(CELLS).fill('unknown');
      const score = new Array<number>(CELLS).fill(5);
      score[idx(4, 4)] = 40;

      render(ended, { heat: score, asOf });

      // the square is a miss on the finished board, but was still open then
      expect(ended.shots[idx(4, 4)]).toBe('miss');
      expect(washes()[idx(4, 4)]).toBeGreaterThan(0);
    });
  });
