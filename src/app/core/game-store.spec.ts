...
/**
 * The Admiral works out a reckoning of every square before each of his salvos
 * and then throws it away. Keeping it is the whole feature; these pin that it is
 * kept, that it is kept only by the level that actually computes one, and that
 * it is thrown out with the game it belonged to.
 */
describe("the Admiral's reckoning", () => {
  // its own clock: this block sits outside the suite above, which owns the other one
  beforeEach(() => {
    vi.useFakeTimers();
    localStorage.clear();
    TestBed.resetTestingModule();
  ...
  /** Play until the opponent has fired at least `salvos` times, by missing on purpose. */
  const fought = (level: Difficulty, salvos = 4, seed = 31337): GameStore => {
    // some tests fight twice, and a TestBed cannot be reconfigured once used
    TestBed.resetTestingModule();
    const store = make(seed);
    ...
    let next = 0;
    while (store.enemyStats().shots < salvos && next < water.length && store.phase() === 'battle') {
      store.fireAt(water[next++]);
      settle();
    }
    return store;
  };

  it('is kept for every salvo the Admiral fires', () => {
    const store = fought('admiral');

    expect(store.reckoning().length).toBeGreaterThan(0);
    for (const frame of store.reckoning()) {
      expect(frame.score).toHaveLength(CELLS);
      expect(frame.shots).toHaveLength(CELLS);
      expect(frame.cell).toBeGreaterThanOrEqual(0);
      // the square it fired at was still open when it chose
      expect(frame.shots[frame.cell]).toBe('unknown');
    }
  });

  it('the reckoning is of the board as it stood, not as it ended', () => {
    const store = fought('admiral');
    const frames = store.reckoning();
    const marksIn = (f: (typeof frames)[number]) =>
      f.shots.filter((state) => state !== 'unknown').length;

    // each salvo is fired at a board with at least as much known as the one before
    for (let i = 1; i < frames.length; i++) {
      expect(marksIn(frames[i])).toBeGreaterThanOrEqual(marksIn(frames[i - 1]));
    }
    expect(marksIn(frames[0])).toBeLessThan(marksIn(frames[frames.length - 1]));
  });

  it('the levels that reckon nothing keep nothing — there would be nothing to show', () => {
    expect(fought('cabin-boy').reckoning()).toHaveLength(0);
    expect(fought('midshipman').reckoning()).toHaveLength(0);
  });

  it('the replay opens on the first salvo and closes again', () => {
    const store = fought('admiral');

    expect(store.replayFrame()).toBeNull();
    store.openReplay();
    expect(store.replayOpen()).toBe(true);
    expect(store.replayFrame()).toBe(store.reckoning()[0]);

    store.showSalvo(2);
    expect(store.replayFrame()).toBe(store.reckoning()[2]);

    store.closeReplay();
    expect(store.replayFrame()).toBeNull();
  });

  it('scrubbing past either end stays on a real salvo', () => {
    const store = fought('admiral');
    store.openReplay();
    const last = store.reckoning().length - 1;

    store.showSalvo(999);
    expect(store.replayAt()).toBe(last);
    store.showSalvo(-5);
    expect(store.replayAt()).toBe(0);
  });

  it('there is nothing to open when nothing was reckoned', () => {
    const store = fought('midshipman');
    store.openReplay();
    expect(store.replayOpen()).toBe(false);
  });

  it('a new game throws the old reckoning away', () => {
    const store = fought('admiral');
    store.openReplay();

    store.newGame();

    expect(store.reckoning()).toHaveLength(0);
    expect(store.replayOpen()).toBe(false);
    expect(store.replayFrame()).toBeNull();
  });
});
